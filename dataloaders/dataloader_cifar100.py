...
from dataloaders.cifar10_dataset import SimSiam_Dataset, TensorDataset
...
def get_cifar100(transform, transform_prime, classes=[50,50], valid_rate = 0.05, seed = 0, batch_size = 128, num_worker = 8):
    pc_valid= valid_rate
    dat = {}
    dat['train']=datasets.CIFAR100('./data/cifar100/',train=True,download=True,transform=transforms.Compose([transforms.ToTensor()]))#normalization removed
    dat['test']=datasets.CIFAR100('./data/cifar100/',train=False,download=True,transform=transforms.Compose([transforms.ToTensor()]))#normalization removed
    data = {}
    lower_bound = 0
    upper_bound = 0
    size=[3,32,32]
    ...
    for k in range(len(classes)):
        ...
        train_data_loaders_pure.append(DataLoader(TensorDataset(xtrain, ytrain), batch_size=batch_size[k], shuffle=True, num_workers = num_worker, pin_memory=True))
