...
def train_cosine_ering_barlow(model, train_data_loaders_generic, knn_train_data_loaders, test_data_loaders, train_data_loaders_linear, device, args, transform, transform_prime):
    epoch_counter = 0
    x_old = torch.Tensor([])
    y_old = torch.tensor([],dtype=torch.long)
    cone_mean = []
    cone_cs = []
    num_arr = [0, 128, 128, 256, 256]
    ...
    for task_id, loader in enumerate(train_data_loaders_generic):
            ...
            for epoch in range(args.epochs[task_id]):
                ...
                for x, _ in loader:   
                    ...
                    if task_id > 0:                  
                        ...
                        indices = np.random.randint(0,x_old.shape[0],sample_num)
                        label_old = y_old[indices]
                        for ind in indices:
                            k = x_old[ind:ind+1].to(device)
                            if args.apply_ering == 1:
                    ...
                    if task_id > 0:
                        ...
                        for ind in range(len(m3_old)):
                            lbl = label_old[ind].item()
                            scores = torch.cosine_similarity(cone_mean[lbl].to(device), m3_old[ind:ind+1])
                            cossine_loss_old += torch.max(torch.tensor(0), cone_cs[lbl]-scores)
                        cossine_loss_old = cossine_loss_old / len(m3_old)

                    if  args.apply_cosine == 1:
                        loss += args.lambdacs * (cossine_loss+15*cossine_loss_old[0])
                ...
                if (epoch+1) % args.knn_report_freq == 0:
                    ...
                    print(f'Task {task_id:2d} | Epoch {epoch:3d} | Time:  {end-start:.1f}s  | Loss: {np.mean(epoch_loss):.4f}  | Cos Loss: {np.mean(coss_loss):.4f}  | Cos Loss Old: {np.mean(coss_loss_old):.4f}  | Knn:  {knn_acc*100:.2f}')
                    print(task_acc_arr)
                else:
                    print(f'Task {task_id:2d} | Epoch {epoch:3d} | Time:  {end-start:.1f}s  | Loss: {np.mean(epoch_loss):.4f}  | Cos Loss: {np.mean(coss_loss):.4f}  | Cos Loss Old: {np.mean(coss_loss_old):.4f} ')
                ...
                wandb.log({" Old Cosine Loss ": np.mean(coss_loss_old), " Epoch ": epoch_counter})  
                wandb.log({" lr ": optimizer.param_groups[0]['lr'], " Epoch ": epoch_counter})
            
            file_name = './checkpoints/checkpoint_' + str(args.dataset) + '-algo' + str(args.appr) + "-e" + str(args.epochs) + "-b" + str(args.pretrain_batch_size) + "-lr" + str(args.pretrain_base_lr) + "-CS" + str(args.class_split) + '_task_' + str(task_id) + '_same_lr_' + str(args.same_lr) + '_norm_' + str(args.normalization) + '_ws_' + str(args.weight_standard)  + '_ering_' + str(args.apply_ering)  + 'cos' + str(args.apply_cosine) + '.pth.tar'
        ...
        x_samp, y_samp = store_samples(loader, task_id, 500)
