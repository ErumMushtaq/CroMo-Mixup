...
def train_cassle_linear_barlow(model, train_data_loaders, knn_train_data_loaders, test_data_loaders, train_data_loaders_linear, device, args):
    ...
    for task_id, loader in enumerate(train_data_loaders):
        ...
        #with torch.no_grad():
        #    linear.weight = nn.parameter.Parameter(torch.eye(512).to(device),requires_grad=False)
        #    linear.bias = nn.parameter.Parameter((torch.rand(512).to(device) * 2 - 1) * 0.25  ,requires_grad=False) 

            #linear.weight = nn.parameter.Parameter()
            #linear.weight = nn.parameter.Parameter(linear.weight/ torch.norm((linear.weight)), requires_grad=False) 
        #    linear.weight = nn.parameter.Parameter(linear.weight/ torch.sqrt(torch.norm((linear.weight)))) 
