...
def train_cassle_noise_barlow(model, train_data_loaders_generic, knn_train_data_loaders, test_data_loaders, train_data_loaders_linear,  transform, transform_prime, device, args):
    ...
    for task_id, loader in enumerate(train_data_loaders_generic):
        ...
        for epoch in range(args.epochs[task_id]):
            ...
            for x, _ in loader:
                ...
                x1.cpu()
                x2.cpu()     
                #contrastive loss with noise
                fake_data = torch.normal(mean = torch.zeros(x3.shape), std = torch.ones(x3.shape)).to(device)
                x3 = x3.to(device)

                data = torch.cat((x3,fake_data))

                x3_label = torch.ones(x3.shape[0], dtype=torch.long) * 0
                fake_label = torch.ones(x3.shape[0], dtype=torch.long) * 1

                label = torch.cat((x3_label, fake_label)).to(device)

                out = model.contrastive_projector(model.encoder.backbone(data).squeeze())

                cross_entry_loss = F.cross_entropy(out,label)

                loss += cross_entry_loss * args.cross_lambda
                

                contrastive_loss.append(cross_entry_loss.item())
            ...
            if (epoch+1) % args.knn_report_freq == 0:
                ...
                print(f'Task {task_id:2d} | Epoch {epoch:3d} | Time:  {end-start:.1f}s  | Loss: {np.mean(epoch_loss):.4f} | Cross Loss: {np.mean(contrastive_loss):.4f} ')
        
            wandb.log({" Average Training Loss ": np.mean(epoch_loss), " Epoch ": epoch_counter, "Contrastive Loss": np.mean(contrastive_loss)})  
        ...
        if task_id < len(train_data_loaders_generic)-1:
            lin_epoch = 100
