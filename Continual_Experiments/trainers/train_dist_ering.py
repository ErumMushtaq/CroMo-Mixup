...
def train_dist_ering_infomax(model, train_data_loaders, knn_train_data_loaders, train_data_loaders_pure, test_data_loaders, device, args, transform, transform_prime):
    memory = torch.Tensor()
    epoch_counter = 0
    # oldModel = model
    oldModel = deepcopy(model.encoder)
    # old_model = None
    for task_id, loader in enumerate(train_data_loaders):
        ...
        if args.resume_checkpoint:
            ...
            oldModel.load_state_dict(model.encoder.state_dict())
            oldModel.to(device)
        ...
        for epoch in range(args.epochs[task_id]):
            ...
            for x1, x2, y in loader:   
                ...
                if task_id > 0:                    
                    ...
                    f1Old = oldModel(xx1).squeeze().detach()
                    f2Old = oldModel(xx2).squeeze().detach()
                ...
                if task_id > 0:
                    cov_loss2 =  covarince_loss(z1[args.pretrain_batch_size:], f2Old)

                if args.info_loss == 'invariance':
                    sim_loss =  invariance_loss(z1, z2)
                    if task_id > 0:
                        # print(f2Old.shape)
                        # print(z1[x1.shape[0]-args.bsize:].shape)
                        sim_loss2 =  invariance_loss(z1[x1.shape[0]-args.bsize:], f2Old)
                elif args.info_loss == 'error_cov': 
                    sim_loss = err_covarince_loss(z1[args.batch_size[task_id]], z2)
                    if task_id > 0:
                        sim_loss2 = err_covarince_loss(z1[args.pretrain_batch_size:[task_id]], f2Old)
                if task_id > 0:    
                    loss = (args.sim_loss_weight * sim_loss) + (args.cov_loss_weight * cov_loss) + args.lambdap*((args.sim_loss_weight * sim_loss2) + (args.cov_loss_weight * cov_loss2))
                else:
