...
def train_LRD_scale_infomax(model, train_data_loaders, knn_train_data_loaders, test_data_loaders, device, args):#just for 2 tasks
    ...
    for task_id, loader in enumerate(train_data_loaders):
        ...
        for epoch in range(args.epochs[task_id]):
            ...
            for x1, x2, y in loader:
                ...
                z1 = model.encoder.projector(f1 * args.scale) # NxC
                z2 = model.encoder.projector(f2 * args.scale) # NxC
                ...
                optimizer.zero_grad()
                loss = loss_task +  args.lambdap * lossKD 
                loss.backward()
            ...
            wandb.log({" Average Training Loss ": np.mean(epoch_loss_task), " Epoch ": epoch_counter, " Average KD Loss ": np.mean(epoch_loss_kd) })  
...
def train_LRD_scale_barlow(model, train_data_loaders, knn_train_data_loaders, test_data_loaders, device, args):#just for 2 tasks
