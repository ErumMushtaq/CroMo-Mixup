...
def train_cassle_cosine_barlow(model, train_data_loaders, knn_train_data_loaders, test_data_loaders, train_data_loaders_linear, device, args):
                ...
                for x1, x2, y in loader:
                    ...
                    if task_id != 0: #do Distillation
                        ...
                        for ind in range(len(cone_cs)):
                            scores1 = torch.cosine_similarity(cone_mean[ind].to(device), m1).detach().cpu()
                            scores2 = torch.cosine_similarity(cone_mean[ind].to(device), m2).detach().cpu()
                            loss += args.lambdacs * (0.5*torch.max(torch.tensor(0), scores1-cone_cs[ind]).mean() + 
